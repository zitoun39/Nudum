...
async function request(url: string, method: string = "GET", body?: any) {
  const token = localStorage.getItem("nudum_token");
  ...
  try {
    ...
    throw err; // propagates the error so the caller can trigger the local database mock
  }
}
...
export const api = {
  ...
  async login(organizationId: string): Promise<{ token: string; user: any }> {
    const token = "mock-jwt-token-signature";
    localStorage.setItem("nudum_token", token);
    const user = {
      id: "usr-1",
  ...
  async logout() {
    localStorage.removeItem("nudum_token");
